...
void* Decoder::thread_handler(void* param){
    ...
    while(true){
		...
		//if(ret_size != temp.secretSize) printf("size inconsistent\n");
 ...
 */
Decoder::Decoder(int type, int n, int m, int r, int securetype, char* filename){
    int i;
    n_ = n;
	...
	char recipeName[256];
	sprintf(recipeName, "%s.meta", filename);
